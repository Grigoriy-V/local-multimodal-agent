...
and by default refuses none (`DECISIONS.md` 2026-09-03). Beside the plan,
and always offered, is the goal (`app/tools/goal.py`, `set_goal`): the
things a request asks for, written down once by the model before it starts
and never updated, so they stay in front of it at every step; nothing reads
it back (`DECISIONS.md` 2026-09-05). Both live the same way. The plan is
