...
The size of the request about to be sent is estimated before every model step (`fitted` in `app/agent/graph.py`), and a conversation over budget is folded before it is sent rather than after the endpoint refuses it. Size is the trigger; a message count (`summarize_after`, 60) is the fallback for a server that reports no window. The summarizer reads the same shortened surface the model reads, never whole tool results, and a fold that fails leaves the turn and its answer as they are. The estimate lives behind the model boundary (`ModelBackend.estimate_tokens`) and calibrates itself from the token counts completions already report. Only stored history folds; shortening the current turn's own accumulated tool results is not implemented, and `ContextOverflowError` remains the backstop under all of it.
