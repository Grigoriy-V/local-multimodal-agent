...
class Server:
    ...
    def start(self) -> None:
        ...
        _warmup()
        # This App's compile cache predates its snapshot, which is the only
        # reason a restore has never failed here: a snapshot holding a handle
        # into a path the container itself wrote on the Volume is not
        # restorable, committed or not (ISS-0047; the Qwen Apps keep their
        # cache off the Volume for that reason). The commit is kept as
        # harmless; the first configuration change that recompiles will need
        # what `model_app_qwen.py` does instead.
        vllm_cache.commit()
        _sleep()
